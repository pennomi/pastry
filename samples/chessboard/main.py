...
def point_at_z(z, point, vec):
    """Given a line (vector plus origin point) and a desired z value, returns
    the point on the line where the desired z value is what we want.
    This is how we know where to position an object in 3D space based on a 2D
    mouse position. It also assumes that we are dragging in the XY plane.

    This is derived from the mathematics of a plane, solved for a given point
    """
    return point + vec * ((z - point.getZ()) / vec.getZ())


def square_pos(i):
    """Get the proper position for a given square"""
    return LPoint3((i % 8) - 3.5, int(i // 8) - 3.5, 0)


def square_color(i):
    """Determine whether a square should be white or black"""
    if (i + ((i // 8) % 2)) % 2:
        return BLACK
    else:
        return WHITE
...
class ChessboardDemo(ShowBase):
    def __init__(self):
        ...
        for i in range(48, 56):
            ...
            self.pieces[i] = Pawn(i, PIECE_BLACK)
        for i in range(8):
            ...
            self.pieces[i + 56] = pieceOrder[i](i + 56, PIECE_BLACK)
